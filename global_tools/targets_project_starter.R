...
required_setup_packages <- c("fs", "here", "renv", "yaml", "withr",
                             "gittargets", "targets", "tarchetypes", "cli")
...
#' create_new_project("Projekt1", renv = FALSE)
#' @export
create_new_project <- function(project_name,
  ...
  create_project_dirs <- function(project_path, dirs) {
    for (d in dirs) {
      fs::dir_create(fs::path(project_path, d))
    }
    invisible(NULL)
  }

  if (fs::dir_exists(project_path)) {
    cli::cli_alert_danger("Project directory already exists: {project_path}")
     stop("Abort")
  }
  ...
  cli::cli_alert_success("Project directory has been created.")
  ...
  write_rproj_file <- function(project_name, project_path) {
    rproj_file <- fs::path(project_path, paste0(project_name, ".Rproj"))
    ...
    cli::cli_alert_success(".Rproj file has been created")
  ...
  write_config_yaml <- function(project_name, project_path, user) {
    ...
    yaml::write_yaml(config, file.path(project_path, "reports", "config.yaml"))
    invisible(NULL)
  }

  write_config_yaml(project_name, project_path, user)
  # cli::cli_alert_success("config.yaml file has been created")
  ...
  write_read_config_script <- function(project_path) {
    read_config_file <- fs::path(project_path, "R", "read_config.R")
    if (!fs::file_exists(read_config_file)) {
      writeLines(
        c(
          "# R/read_config.R",
          "",
          ...
          "    cli::cli_alert_warning('config.yaml not found, default info will be used.')",
  ...
  # cli::cli_alert_success("read_config.R file has been created")
  ...
  write_quarto_config <- function(project_path) {
    ...
    yaml::write_yaml(quarto_config, file.path(project_path, "reports", "_quarto.yml"))
    invisible(NULL)
  }

  write_quarto_config(project_path)
  # cli::cli_alert_success("_quarto.yml file has been created")
  ...
  write_report_template <- function(project_name, project_path) {
    report_file <- fs::path(project_path, "reports", paste0(project_name, "_report.qmd"))
    if (!fs::file_exists(report_file)) {
      writeLines(
        c(
          "---",
  ...
  # cli::cli_alert_success(project_name, "_report.qmd has been created")
  ...
  write_load_lib_script <- function(project_path,
    ...
    lib_file <- fs::path(project_path, "R", "load_lib.R")
    if (!fs::file_exists(lib_file)) {
      writeLines(
        c(
          ...
          "      install.packages(pkg)",
          "    }, error = function(e) {",
          "      cli::cli_alert_warning('Package could not be installed: ', pkg)",
          "    })",
          "  }",
          "}",
          "",
          ...
          "cli::cli_alert_success('Alle Bibliotheken wurden geladen.')",
          "",
  ...
  # cli::cli_alert_success('load_lib.R was created')
  ...
  write_readme <- function(project_name, project_path, user) {
    readme_file <- fs::path(project_path, "README.qmd")
    today <- format(Sys.Date(), "%Y-%m-%d")
    writeLines(
      c(
        "---",
        ...
        "- `tar_visnetwork()` – display dependency graph",
  ...
  # cli::cli_alert_success("README.qmd file has been created")
  ...
  create_notebook <- function(project_name, project_path, user) {
    today <- format(Sys.Date(), "%Y-%m-%d")
    notebook_today <- fs::path(project_path, "notebook", paste0(today, "_notes.qmd"))
    results_today <- fs::path(project_path, "results", paste0(today, "_initial"))

    if (!fs::dir_exists(results_today)) fs::dir_create(results_today)
    if (!fs::file_exists(notebook_today)) {
      writeLines(
        c(
          "---",
  ...
  # cli::cli_alert_success("notebook file has been created")
  ...
  initialize_renv_env <- function(project_path, renv = TRUE) {
    if (!renv) {
      cli::cli_alert_success("Project was created without using renv.")
      return(invisible(NULL))
    }
    if (!requireNamespace("renv", quietly = TRUE)) {
      cli::cli_alert_danger("renv was not initialized. Project will be created without renv.")
      return(invisible(NULL))
    }
    ...
    cli::cli_alert_success("renv was initialized. Please use `renv::snapshot()` after a package installation.")
    invisible(NULL)
  }

  initialize_renv_env(project_path, renv = renv)
  ...
  write_runall_script <- function(project_name, project_path) {
    runall_file <- fs::path(project_path, "R", "runall.R")

    if (targets == FALSE) {

      if (!fs::file_exists(runall_file)) {
        writeLines(
          c(
    ...
    invisible(NULL)
    cli::cli_alert_success("runall.R file has been created")
    }
  }
  ...
  cleaning_pipeline <- file.path(project_path, "R", "cleaning", "pipeline.R")
  cleaning_file <- file.path(project_path, "R", "cleaning", "1_preparation.R")
  today <- format(Sys.Date(), "%Y-%m-%d")

  if (!fs::file_exists(cleaning_pipeline)) {
    writeLines(
      c(
        "",
        "# pipeline.R",
    ...
    con = cleaning_pipeline
    )
  }


  if (!fs::file_exists(cleaning_file)) {
    writeLines(
      c(
        "",
        "# 1_preparation.R",
  ...
  # cli::cli_alert_success("cleaning_data_pipeline.R script has been created")
  ...
  write_test_structure <- function(project_path) {
    testthat_dir <- fs::path(project_path, "tests", "testthat")
    fs::dir_create(testthat_dir)
    # Simple datacheck
    test_data_file <- fs::path(testthat_dir, "test_data_checks.R")
    if (!fs::file_exists(test_data_file)) {
      writeLines(
        c(
    ...
    # Pipeline-Test
    pipeline_test_file <- fs::path(testthat_dir, "test_pipeline.R")
    if (!fs::file_exists(pipeline_test_file)) {
      writeLines(
        c(
  ...
  # cli::cli_alert_success("Test that skeleton has been created")
  ...
  write_gitignore <- function(project_path) {
    gitignore_file <- fs::path(project_path, ".gitignore")
    gitignore_content <- c(
      "# R",
      ".Rhistory",
      ".RData",
  ...
  # cli::cli_alert_success(".gitignore file has been created")
  ...
  write_targets_skeleton <- function(project_path, project_name) {
    ...
    if (targets) {
      targets_file <- fs::path(project_path, "_targets.R")

      if (!fs::file_exists(targets_file)) {
        writeLines(
          c(
            "### ____________________________________________________________________________",
            "#",
            ...
            "",
            "### ------ cleaning script -----------------------------",
            "tarchetypes::tar_source(",
            "  cleaning_scripts,",
            "",
            "    files = c('R/cleaning/pipeline.R', 'R/cleaning/1_preparation.R')",
            "",
            "),",
            "### ____________________________________________________",
            "",
      ...
      invisible(NULL)
      cli::cli_alert_success("_targets.R file has been created")
    }
  }


  write_targets_skeleton(project_path, project_name)
  ...
  if (!file.exists(renviron_path)) {
    ...
    # cli::cli_alert_success(".Renviron file was created for keys and tokens")
  }
  ...
  cli::cli_alert_success("Project '{project_name}' was created in: {project_path}")
